...
class SettingService(private val repository: SettingRepository) {
    ...
    fun update(request: SettingResponse): Setting {
        val setting = get()
        setting.workingHours = request.workingHours.toDuration()
            ?: throw BadRequestException("Invalid working hours")

        setting.breakTime = request.breakTime.toDuration()
            ?: throw BadRequestException("Invalid break time")
